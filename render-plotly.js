...
import { LEGEND_ITEMS, CHART_CONFIG } from './src/sankey-config.js';
...
function findLatestPlotlyData() {
  ...
  const files = fs.readdirSync(dataDir)
    ...
    .map(f => ({
      name: f,
      mtime: fs.statSync(join(dataDir, f)).mtime
    }))
    .sort((a, b) => b.mtime - a.mtime);

  if (files.length === 0) {
    throw new Error('No plotly-sankey-*.json files found. Run "npm run build" first.');
  }

  const filePath = join(dataDir, files[0].name);
